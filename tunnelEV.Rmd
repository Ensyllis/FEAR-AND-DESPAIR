...
```{}
computeCenterCoordinateAndAddVerticalShift <- function(dataframe) {
  

  # (.2795, 3.61) is the point given for the "perfect" fastball, this can change if needed
  dataff$radius = sqrt(((.2795 - dataff$plate_x)^2) + ((3.61 - dataff$plate_z)^2))

  radius = sqrt(((.2795 - dataff$plate_x)^2) + ((3.61 - dataff$plate_z)^2))
  
  radius = mean(radius, na.rm = TRUE)
  
  # Further filter data to pitches that land in our two-ball zone
  datafin = filter(dataff, radius < 0.1431285)
  ...
  # Calculate averages
  xavg = mean(xcords, na.rm = TRUE)
  zavg = mean(zcords, na.rm = TRUE)

  # Append the averages as new columns
  dataframe$xavg = xavg
  dataframe$zavg = zavg
  dataframe$radius = radius

  # Add vertical shift
  dataframe$zavg <- dataframe$zavg + (7 / 60) * (dataframe$release_speed - 80)
...
```{}
testdf <- computeCenterCoordinateAndAddVerticalShift(dat.wo, "Four.Seam","R")
```
...
findCenterOfPitch3 <- function(dataframe, x1, pthrow) {
  
      # Define the base speeds for each pitch type
  base_speeds <- c(Four.Seam = 93.3728, Two.Seam = 92.4290, Slide.Cutter = 85.6178, ChangeUp.Splitter = 84.5143, Curveball = 78.3268)
  
  # Get the base speed for the given pitch type
  base_speed <- base_speeds[x1]
  # Filter data to just 4-seam fastballs between 94-96mph
  
  dataff = filter(dataframe, legible_pitch_type ==  x1 & release_speed > (base_speed - 1) & release_speed < (base_speed + 1) & p_throws == pthrow)
...
testdf1 <- findCenterOfPitch3(dataff, "Two.Seam","R")
```
...
findCenterOfPitch2 <- function(data, pitch_type, pitcherMatchUp) {
  # Filter data to the specified pitch type and pitcher's throwing arm
  data %>% 
    filter(legible_pitch_type == pitch_type, p_throws == pitcherMatchUp) %>%
    
    # Calculate the radius from the 'perfect' pitch location (.2795, 3.61)
    mutate(radius = sqrt((.2795 - plate_x)^2 + (3.61 - plate_z)^2)) %>%
    
    # Filter data to pitches that land within a specified radius
    filter(radius < 0.1431285) %>%
    
    # Calculations for lines
    mutate(tval = (release_pos_y - 1.417) / -(vy0 - release_pos_y),
           xcords = ((vx0 - release_pos_x) * tval) + release_pos_x,
           zcords = ((vz0 - release_pos_z) * tval) + release_pos_z) %>%
    
    # Calculate averages
    summarise(xavg = mean(xcords, na.rm = TRUE),
              zavg = mean(zcords, na.rm = TRUE),
              release_speed_avg = mean(release_speed, na.rm = TRUE)) %>%
    
    # Add vertical shift based on average release speed
    mutate(zavg_shifted = zavg + (7 / 60) * (release_speed_avg - 80)) %>%
    
    # Return the result
    select(xavg, zavg, zavg_shifted)
}
```
...
findCenterOfPitch <- function(data, pitch_type, pitcherMatchUp)
{
    ...
    dataff = filter(data, legible_pitch_type == pitch_type & release_speed > (base_speed - 1) &          release_speed < (base_speed + 1) & p_throws == pitcherMatchUp)
    ...
    # Ipmportant. We may be unable to do batter individual tunnels 
    # Because it's incredibly hard to throw a ball through said tunnel.
    # Due to this we may need to increase the radius/tolernace level of the tunnel
    
    ################################################################
    # Further filter data to pitches that land in our two-ball zone
    datafin = filter(dataff, radius < 1)
...
findCenterOfPitch <- function(data, pitch_type, pitcherMatchUp) {
    ...
    # Further filter data to pitches that land in our two-ball zone
    datafin <- filter(dataff, radius < 0.286257)
...
filter_in_tunneling <- function(data, threshold = 0.1431285) {
...
filter_out_tunneling <- function(data, threshold = 0.1431285) {
  # Directly filter data based on the distance calculation
  in_tunnel_data <- data %>%
    filter(sqrt((plate_x - xavg)^2 + (universal_plate_z - zavg)^2) > threshold)
...
```{r}
generate_tunnel_statistics <- function(data, pitch) {
  ...
  # Construct captions for in-tunnel statistics
  in_tun_caption <- paste(
    "In-Tunnel Hit%:", roundNA(in_tun_tunnel_hit_percentage * 100, 2),
    "% | In-Tunnel Batting Average:", roundNA(in_tun_batting_average * 100, 2),
    "% | In-Tunnel Well-Hitting Average:", roundNA(in_tun_WellHittingAverage * 100, 2), "%"
  )
  
  in_tun_caption2 <- paste(
    "In-Tunnel Swing/Miss%:", roundNA(in_tun_SwingMissPercentage * 100, 2),
    "% | In-Tunnel Home Run%:", roundNA(in_tun_HomeRunPercentage * 100, 2),
    "% | In-Tunnel Ground Ball%:", roundNA(in_tun_GroundBallPercentage * 100, 2), "%"
  )

  # Construct captions for out-tunnel statistics
  out_tun_caption <- paste(
    "Out-Tunnel Hit%:", roundNA(out_tun_tunnel_hit_percentage * 100, 2),
    "% | Out-Tunnel Batting Average:", roundNA(out_tun_batting_average * 100, 2),
    "% | Out-Tunnel Well-Hitting Average:", roundNA(out_tun_WellHittingAverage * 100, 2), "%"
  )
  
  out_tun_caption_2 <- paste(
        "Out-Tunnel Swing/Miss%:", roundNA(out_tun_SwingMissPercentage * 100, 2),
    "% | Out-Tunnel Home Run%:", roundNA(out_tun_HomeRunPercentage * 100, 2),
    "% | Out-Tunnel Ground Ball%:", roundNA(out_tun_GroundBallPercentage * 100, 2), "%"
  )

  # Combine the total, in-tunnel, and out-tunnel captions
  total_caption <- paste(
    "Total Hit%:", roundNA(total_hit_percentage * 100, 2), "%"
  )

  returned_caption <- paste(total_caption, in_tun_caption, in_tun_caption2, out_tun_caption, out_tun_caption_2, sep="\n\n")

  return(returned_caption)
}
```
